...
static bool callValue(Value callee, int argCount)
{
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            case OBJ_CLOSURE:
                return call_r(AS_CLOSURE(callee), argCount);
            ...
            case OBJ_CLASS: {
                ...
                if (tableGet(&klass->methods, vm.initString, &initializer)) {
                    return call_r(AS_CLOSURE(initializer), argCount);
                } else if (argCount != 0) {
            ...
            case OBJ_BOUND_METHOD: {
                ...
                return call_r(bound->method, argCount);
            }
...
static bool invokeFromClass(ObjClass* klass, ObjString* name, int argCount)
{
    ...
    return call_r(AS_CLOSURE(method), argCount);
}
